/* tslint:disable max-line-length */
import { TestBed, getTestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';
import { HttpClient, HttpResponse } from '@angular/common/http';
import { of } from 'rxjs';
import { take, map } from 'rxjs/operators';
import * as moment from 'moment';
import { DATE_FORMAT } from 'app/shared/constants/input.constants';
import { GiftService } from 'app/entities/gift/gift.service';
import { IGift, Gift } from 'app/shared/model/gift.model';

describe('Service Tests', () => {
    describe('Gift Service', () => {
        let injector: TestBed;
        let service: GiftService;
        let httpMock: HttpTestingController;
        let elemDefault: IGift;
        let currentDate: moment.Moment;
        beforeEach(() => {
            TestBed.configureTestingModule({
                imports: [HttpClientTestingModule]
            });
            injector = getTestBed();
            service = injector.get(GiftService);
            httpMock = injector.get(HttpTestingController);
            currentDate = moment();

            elemDefault = new Gift(0, 0, currentDate, 0, 0, 0, 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', 'AAAAAAA', 0, 0, 0);
        });

        describe('Service methods', async () => {
            it('should find an element', async () => {
                const returnedFromService = Object.assign(
                    {
                        orderedAt: currentDate.format(DATE_FORMAT)
                    },
                    elemDefault
                );
                service
                    .find(123)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: elemDefault }));

                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should create a Gift', async () => {
                const returnedFromService = Object.assign(
                    {
                        id: 0,
                        orderedAt: currentDate.format(DATE_FORMAT)
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        orderedAt: currentDate
                    },
                    returnedFromService
                );
                service
                    .create(new Gift(null))
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'POST' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should update a Gift', async () => {
                const returnedFromService = Object.assign(
                    {
                        amount: 1,
                        orderedAt: currentDate.format(DATE_FORMAT),
                        fee: 1,
                        exchangeRate: 1,
                        totalAmount: 1,
                        sendFrom: 'BBBBBB',
                        message: 'BBBBBB',
                        referenceNumber: 'BBBBBB',
                        depositor: 'BBBBBB',
                        displayAt: 'BBBBBB',
                        senderId: 1,
                        recipientId: 1,
                        giftcardId: 1
                    },
                    elemDefault
                );

                const expected = Object.assign(
                    {
                        orderedAt: currentDate
                    },
                    returnedFromService
                );
                service
                    .update(expected)
                    .pipe(take(1))
                    .subscribe(resp => expect(resp).toMatchObject({ body: expected }));
                const req = httpMock.expectOne({ method: 'PUT' });
                req.flush(JSON.stringify(returnedFromService));
            });

            it('should return a list of Gift', async () => {
                const returnedFromService = Object.assign(
                    {
                        amount: 1,
                        orderedAt: currentDate.format(DATE_FORMAT),
                        fee: 1,
                        exchangeRate: 1,
                        totalAmount: 1,
                        sendFrom: 'BBBBBB',
                        message: 'BBBBBB',
                        referenceNumber: 'BBBBBB',
                        depositor: 'BBBBBB',
                        displayAt: 'BBBBBB',
                        senderId: 1,
                        recipientId: 1,
                        giftcardId: 1
                    },
                    elemDefault
                );
                const expected = Object.assign(
                    {
                        orderedAt: currentDate
                    },
                    returnedFromService
                );
                service
                    .query(expected)
                    .pipe(
                        take(1),
                        map(resp => resp.body)
                    )
                    .subscribe(body => expect(body).toContainEqual(expected));
                const req = httpMock.expectOne({ method: 'GET' });
                req.flush(JSON.stringify([returnedFromService]));
                httpMock.verify();
            });

            it('should delete a Gift', async () => {
                const rxPromise = service.delete(123).subscribe(resp => expect(resp.ok));

                const req = httpMock.expectOne({ method: 'DELETE' });
                req.flush({ status: 200 });
            });
        });

        afterEach(() => {
            httpMock.verify();
        });
    });
});
